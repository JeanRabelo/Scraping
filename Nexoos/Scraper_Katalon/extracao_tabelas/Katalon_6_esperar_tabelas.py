...
print(f'str_email = jeanpablosousarabelo@gmail.com')
print(f'str_password = 9@VGn%Pr6!b7bwhXD8UQzih^ynSj!i')
saida = open('saida_tabelas.csv', 'w', newline='')
...
while pagina <=199:
    try:
        driver.find_element_by_xpath("//a[contains(@href, '/mkt/lender?loans_funded=" + str(pagina) + "&view_mode=list')]").click()
    except:
        try:
            sleep(2)
            driver.find_element_by_xpath("//a[contains(@href, '/mkt/lender?loans_funded=" + str(pagina) + "&view_mode=list')]").click()
